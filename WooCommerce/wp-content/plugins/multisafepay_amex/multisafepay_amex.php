...
/*
  Plugin Name: Multisafepay American Express
...
if (!function_exists('is_woo_page')) {
...
if (!function_exists('is_woo_checkout')) {

    function is_woo_checkout() {
        return is_the_page(is_woo_page('checkout')) || apply_filters('woocommerce_is_checkout', false) ? true : false;
    }

}

if (!function_exists('is_the_page')) {

    function is_the_page($id) {
...
if (/* is_shop2() || */ is_woo_checkout()/* || is_cart2() */ || is_admin()) {
...
if ($activate_plugin) {
    ...
    if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
        add_action('plugins_loaded', 'WC_MULTISAFEPAY_AMEX_Load', 0);

        function WC_MULTISAFEPAY_AMEX_Load() {

            class WC_MULTISAFEPAY_AMEX extends WC_Payment_Gateway {
                ...
                public function __construct() {
                    ...
                    $this->id = "MULTISAFEPAY_AMEX";
                    $this->has_fields = false;
                    $this->paymentMethodCode = "AMEX";
                    $this->supports = array(
                        /* 'subscriptions',
                          'products',
                    ...
                    add_filter('woocommerce_payment_gateways', array('WC_MULTISAFEPAY_AMEX', 'MULTISAFEPAY_AMEX_Add_Gateway'));
                    ...
                    if ($this->settings['pmtitle'] != "") {
                        ...
                        $this->title = "American Express";
                        $this->method_title = "American Express";
                    }
...
//$woocommerce->add_error(__('Payment error:', 'multisafepay') . ' ' . $msp->error);
                ...
                public static function MULTISAFEPAY_AMEX_Add_Gateway($methods) {
                    global $woocommerce;
                    $methods[] = 'WC_MULTISAFEPAY_AMEX';
